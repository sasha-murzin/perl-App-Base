...
--quux=N           quux is an integer option (default: 7)                                             
            ...
            $ENV{COLUMNS} = 100;
            $long_switches =~ s/\s+/ /g;
            is( trim($long_switches), trim($scswitches),
                'COLUMNS environment variable controls width of switch table output' );
        }
      ...
      NONSENSE:
        {
            ...
            warnings_like {
                exits_ok( sub { Test::Script::Common->new; },
                    "Can't instantiate a script with an invalid option" );
            }
